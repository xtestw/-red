...
-- 自定义策略表
CREATE TABLE IF NOT EXISTS `custom_strategy` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `name` varchar(100) NOT NULL COMMENT '策略名称',
  `description` text COMMENT '策略描述',
  `sql_query` text NOT NULL COMMENT 'SQL查询语句',
  `missing_tables` text COMMENT '缺失的数据表（JSON格式）',
  `execution_rule` varchar(50) DEFAULT NULL COMMENT '执行规则：daily（每天）、weekly（每周）、monthly（每月）',
  `execution_time` varchar(20) DEFAULT NULL COMMENT '执行时间，格式：HH:MM，如15:30',
  `is_active` int(11) DEFAULT 1 COMMENT '是否启用：1-启用，0-禁用',
  `created_at` datetime DEFAULT NULL COMMENT '创建时间',
  `updated_at` datetime DEFAULT NULL COMMENT '更新时间',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='自定义策略表';

-- 策略胜率表
CREATE TABLE IF NOT EXISTS `strategy_win_rate` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `strategy_name` varchar(100) NOT NULL COMMENT '策略名称',
  `trade_date` varchar(10) NOT NULL COMMENT '选股日期',
  `win_rate_1d` float DEFAULT NULL COMMENT '次日胜率（%）',
  `win_rate_3d` float DEFAULT NULL COMMENT '3日胜率（%）',
  `win_rate_5d` float DEFAULT NULL COMMENT '5日胜率（%）',
  `win_rate_10d` float DEFAULT NULL COMMENT '10日胜率（%）',
  `win_rate_20d` float DEFAULT NULL COMMENT '20日胜率（%）',
  `total_count` int(11) DEFAULT NULL COMMENT '总选股数量',
  `win_count_1d` int(11) DEFAULT NULL COMMENT '次日有效选股数量',
  `win_count_3d` int(11) DEFAULT NULL COMMENT '3日有效选股数量',
  `win_count_5d` int(11) DEFAULT NULL COMMENT '5日有效选股数量',
  `win_count_10d` int(11) DEFAULT NULL COMMENT '10日有效选股数量',
  `win_count_20d` int(11) DEFAULT NULL COMMENT '20日有效选股数量',
  `historical_win_rate_1d` float DEFAULT NULL COMMENT '历史累计次日胜率（%）',
  `historical_win_rate_3d` float DEFAULT NULL COMMENT '历史累计3日胜率（%）',
  `historical_win_rate_5d` float DEFAULT NULL COMMENT '历史累计5日胜率（%）',
  `historical_win_rate_10d` float DEFAULT NULL COMMENT '历史累计10日胜率（%）',
  `historical_win_rate_20d` float DEFAULT NULL COMMENT '历史累计20日胜率（%）',
  `historical_total_count` int(11) DEFAULT NULL COMMENT '历史累计总选股数量',
  `historical_win_count_1d` int(11) DEFAULT NULL COMMENT '历史累计次日有效选股数量',
  `historical_win_count_3d` int(11) DEFAULT NULL COMMENT '历史累计3日有效选股数量',
  `historical_win_count_5d` int(11) DEFAULT NULL COMMENT '历史累计5日有效选股数量',
  `historical_win_count_10d` int(11) DEFAULT NULL COMMENT '历史累计10日有效选股数量',
  `historical_win_count_20d` int(11) DEFAULT NULL COMMENT '历史累计20日有效选股数量',
  `rolling_5d_win_rate_1d` float DEFAULT NULL COMMENT '过去5日次日胜率（%）',
  `rolling_5d_win_rate_3d` float DEFAULT NULL COMMENT '过去5日3日胜率（%）',
  `rolling_5d_win_rate_5d` float DEFAULT NULL COMMENT '过去5日5日胜率（%）',
  `rolling_5d_win_rate_10d` float DEFAULT NULL COMMENT '过去5日10日胜率（%）',
  `rolling_5d_win_rate_20d` float DEFAULT NULL COMMENT '过去5日20日胜率（%）',
  `rolling_10d_win_rate_1d` float DEFAULT NULL COMMENT '过去10日次日胜率（%）',
  `rolling_10d_win_rate_3d` float DEFAULT NULL COMMENT '过去10日3日胜率（%）',
  `rolling_10d_win_rate_5d` float DEFAULT NULL COMMENT '过去10日5日胜率（%）',
  `rolling_10d_win_rate_10d` float DEFAULT NULL COMMENT '过去10日10日胜率（%）',
  `rolling_10d_win_rate_20d` float DEFAULT NULL COMMENT '过去10日20日胜率（%）',
  `rolling_20d_win_rate_1d` float DEFAULT NULL COMMENT '过去20日次日胜率（%）',
  `rolling_20d_win_rate_3d` float DEFAULT NULL COMMENT '过去20日3日胜率（%）',
  `rolling_20d_win_rate_5d` float DEFAULT NULL COMMENT '过去20日5日胜率（%）',
  `rolling_20d_win_rate_10d` float DEFAULT NULL COMMENT '过去20日10日胜率（%）',
  `rolling_20d_win_rate_20d` float DEFAULT NULL COMMENT '过去20日20日胜率（%）',
  `rolling_30d_win_rate_1d` float DEFAULT NULL COMMENT '过去30日次日胜率（%）',
  `rolling_30d_win_rate_3d` float DEFAULT NULL COMMENT '过去30日3日胜率（%）',
  `rolling_30d_win_rate_5d` float DEFAULT NULL COMMENT '过去30日5日胜率（%）',
  `rolling_30d_win_rate_10d` float DEFAULT NULL COMMENT '过去30日10日胜率（%）',
  `rolling_30d_win_rate_20d` float DEFAULT NULL COMMENT '过去30日20日胜率（%）',
  `rolling_60d_win_rate_1d` float DEFAULT NULL COMMENT '过去60日次日胜率（%）',
  `rolling_60d_win_rate_3d` float DEFAULT NULL COMMENT '过去60日3日胜率（%）',
  `rolling_60d_win_rate_5d` float DEFAULT NULL COMMENT '过去60日5日胜率（%）',
  `rolling_60d_win_rate_10d` float DEFAULT NULL COMMENT '过去60日10日胜率（%）',
  `rolling_60d_win_rate_20d` float DEFAULT NULL COMMENT '过去60日20日胜率（%）',
  `rolling_90d_win_rate_1d` float DEFAULT NULL COMMENT '过去90日次日胜率（%）',
  `rolling_90d_win_rate_3d` float DEFAULT NULL COMMENT '过去90日3日胜率（%）',
  `rolling_90d_win_rate_5d` float DEFAULT NULL COMMENT '过去90日5日胜率（%）',
  `rolling_90d_win_rate_10d` float DEFAULT NULL COMMENT '过去90日10日胜率（%）',
  `rolling_90d_win_rate_20d` float DEFAULT NULL COMMENT '过去90日20日胜率（%）',
  `created_at` datetime DEFAULT NULL COMMENT '创建时间',
  `updated_at` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_strategy_date` (`strategy_name`, `trade_date`),
  KEY `idx_strategy_name` (`strategy_name`),
  KEY `idx_trade_date` (`trade_date`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='策略胜率表';
